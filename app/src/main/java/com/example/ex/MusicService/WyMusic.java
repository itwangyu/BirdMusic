...
import com.example.ex.MusicBean.wy.NeteaseDatas;
import com.example.ex.MusicBean.wy.NeteaseLostSong;
import com.example.ex.MusicBean.wy.NeteaseLostSongAlbumId;
import com.example.ex.MusicBean.wy.NeteaseLrc;
import com.example.ex.MusicBean.wy.NeteaseMv;
import com.example.ex.MusicBean.wy.NeteasePic;
import com.example.ex.MusicBean.wy.NeteaseSongUrl;
...
public class WyMusic implements IMusic {
    ...
    //网易不支持无损解析，支持下架音乐
    private static ResultBean search(String key, int page, int size) {
        try {
            ...
            return new ResultBean(totalsize ,songResults);
        } catch (Exception e) {
            return null;//解析歌曲错误
        }
    }
    ...
    @Override
    public ResultBean SongSearch(String key, int page, int size) {
